...
public class pesanan extends item{
    ...
    private String getNomorEvent(int id){
        if (eventStatus(id)) {
            int nomor = Integer.parseInt(getCurrentEvent()[0]);
            if (nomor < 10) {
                return "00" + nomor;
            } else if (nomor < 100) {
                return "0" + nomor;
    ...
    public void inputDataQR(String nomor) throws WriterException, IOException {
        ...
        String qrCodeText =
                ...
                "\nNama barang       = " + getDataItem(Integer.parseInt(dataPesanan[4]))[1]   +
                "\nJumlah barang     = " + dataPesanan[5]                                          +
                "\nHarga             = " + dataPesanan[6]                                          +
                "\nAlamat            = " + account.getDataAccount(dataPesanan[2])[7];

        String filePath = nomor +".png";
        int size = 400;
        File qrFile = new File(filePath);
        createQRImage(qrFile, qrCodeText, size, "png");
    }
